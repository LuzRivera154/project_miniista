...
if (isset($_FILES["archivo"]) && isset($_POST["autor"])) {
    ...
    // Formato: 20250601144419-pierre-video.mp4
    $nuevo_nombre    = "$fecha-$autor-$nombre_original";
    $carpeta_destino = __DIR__ . "/uploads/videos/";
    ...
    if ($codigo_error === UPLOAD_ERR_OK) {
        ...
        if ($subida_exitosa) {
            ...
            $archivo_subido = [
                ...
                "src"           => "uploads/videos/$nuevo_nombre",
            ];
        }
    }
}
...
<body>
    ...
    <main class="pagina-upload">
        ...
        <?php if ($subida_exitosa): ?>
            ...
            <div class="tarjeta-foto">
                <video controls>
                    <source src="<?= htmlspecialchars($archivo_subido["src"]) ?>" type="<?= htmlspecialchars($archivo_subido["tipo_mime"]) ?>">
                </video>
        ...
        <?php else: ?>
            ...
            <h2 class="titulo-form">Ajouter une vidéo</h2>

            <form action="upload-video.php" method="post" enctype="multipart/form-data" id="form-video">
                <input id="campo-video" type="file" name="archivo" accept="video/*">
                <label id="label-video" class="btn-archivo" for="campo-video">
                ...
                <span id="nombre-video" class="nombre-archivo">Aucun fichier</span>
    ...
    <nav class="nav-bottom">
        ...
        <a href="upload-video.php" class="nav-icono activo" title="Ajouter une vidéo">
            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <polygon points="23 7 16 12 23 17 23 7"/>
                <rect x="1" y="5" width="15" height="14" rx="2" ry="2"/>
            </svg>
    ...
    <script>
        var inputVideo   = document.getElementById("campo-video");
        var nombreVideo  = document.getElementById("nombre-video");
        ...
        if (inputVideo) {
            inputVideo.addEventListener("change", function() {
                var archivo = inputVideo.files[0];
                if (archivo) {
                    nombreVideo.textContent = archivo.name;
                    ...
                    nombreVideo.textContent = "Aucun fichier";
                }
            });
        }
        ...
        if (formVideo) {
            formVideo.addEventListener("submit", function(e) {
                if (!inputVideo.files || inputVideo.files.length === 0) {
                    e.preventDefault();
                    console.error("Formulario bloqueado: no se seleccionó ningún video");
